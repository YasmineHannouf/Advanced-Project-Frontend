...
import Add from "./PopUp/IncomesPopUp"; //CloseButton
...
export default function BasicTable() {
  ...
  const handleDelete = (rowsDeleted) => {
    ...
    axios.delete(`http://127.0.0.1:8000/api/Incomes/delete/${rowsDeleted}`)
    .then((response) => {
  ...
  const getData = () => axios
  .get("http://127.0.0.1:8000/api/Incomes/show")
  .then((response) => {
    setIncomes(response.data.Fixed_keysByDesc);
    setIsLoading(false);
    console.log(response);
  })
  .catch((error) => {
  ...
  const handleUpdate = (rowData) => {
   ...
   axios.patch(`http://127.0.0.1:8000/api/Incomes/update/${rowData[0]}`, {name:rowData[1],is_active:rowData[2]})
    .then((response) => {
      getData();
      console.log(rowData);
    })
